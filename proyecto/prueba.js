...
function setupChatbot() {
    ...
    function getBotResponse(message) {
        ...
        if (lowerMessage.includes('hola') || lowerMessage.includes('buenos días') || lowerMessage.includes('buenas tardes')) {
            return '¡Hola! Soy el asistente virtual de SIAMsoft. ¿En qué puedo ayudarte hoy?';
        } else if (lowerMessage.includes('catastro')) {
            return 'Nuestro sistema de catastro urbano permite gestionar y administrar el catastro utilizando tecnología de última generación. ¿Te gustaría más información?';
        } else if (lowerMessage.includes('precio') || lowerMessage.includes('costo')) {
            return 'Los precios varían según las necesidades de tu municipalidad. ¿Podrías proporcionarme más detalles sobre lo que necesitas?';
        } else if (lowerMessage.includes('contacto') || lowerMessage.includes('whatsapp')) {
            return 'Puedes contactarnos al +51 996 909 051 o escribirnos a rs.siamsoft@gmail.com';
        } else if (lowerMessage.includes('gracias')) {
            return '¡De nada! Estoy aquí para ayudarte. ¿Hay algo más en lo que pueda asistirte?';
        } else {
            return 'Gracias por tu mensaje. Un especialista de SIAMsoft se contactará contigo pronto para brindarte más información.';
        }
    }
}
